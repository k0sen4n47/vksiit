...
class StudentController extends Controller
{
    ...
    public function storeStudent(Request $request)
    {
        ...
        // Генерация логина
        $shortNamePart = $this->transliterateCyrillicToLatin($group->short_name); // Используем метод транслитерации
        $fioParts = explode(' ', trim($data['fio']));
        $lastName = $this->transliterateCyrillicToLatin(array_shift($fioParts));
        $firstLetterName = !empty($fioParts) ? $this->transliterateCyrillicToLatin(mb_substr(array_shift($fioParts), 0, 1, 'UTF-8')) : '';
        $firstLetterMiddleName = !empty($fioParts) ? $this->transliterateCyrillicToLatin(mb_substr(array_shift($fioParts), 0, 1, 'UTF-8')) : '';
    ...
    {
        // Здесь можно получить данные, специфичные для студента (например, его группы, предметы и т.д.)
     ...
     * @return string
     */
    public static function transliterateCyrillicToLatin(string $string): string
    {
        $converter = array(
            'а' => 'a',   'б' => 'b',   'в' => 'v',   'г' => 'g',   'д' => 'd',
