...
class GroupController extends Controller
{
    ...
    public function storeGroup(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'suffix' => 'nullable|string|max:255', // Суффикс необязателен (добавлено обратно)
        ...
        $nameComponent = GroupNameComponent::find($data['name_component_id']);

        // Формируем полное и сокращенное название группы с учетом суффикса из формы
        ...
        if ($nameValidator->fails()) {
            return redirect()->back()
                        ...
                        ->withInput($request->all()); // Сохраняем все введенные данные
        }
        ...
        $group = Group::create([
            ...
            'course' => $data['course'],
            'year' => $data['year'],
            'suffix' => $data['suffix'] ?? null, // Используем суффикс из формы
            'curator_id' => $data['curator_id'] ?? null,
        ]);

        // Привязка студентов к группе
        if (!empty($data['students'])) {
            // Обновляем поле group_id у выбранных студентов
    ...
    public function updateGroup(Request $request, Group $group)
    {
        ...
        // Сначала отвязываем всех студентов от этой группы
        User::where('group_id', $group->id)->update(['group_id' => null]);

        // Затем привязываем выбранных студентов к группе
        if (!empty($data['students'])) {
            User::whereIn('id', $data['students'])->update(['group_id' => $group->id]);
        }
